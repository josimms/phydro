...
ggplot() + 
  ...
  labs(color = "Model Versions and\nInfrastructure Value",
       x = expression(PPFD~(μmol~m^-2~s^-1))) +
...
ggplot(df_carb_allocation, aes(y=value, x=carb_allocation)) + 
  ...
  theme(strip.background = element_rect(color = "white", size = 1),
        ...
        strip.text = element_text(size = 10, face = "bold"),
        axis.title.y = element_blank()) +
  labs(x = "Infrastructure Parameter") +
  facet_wrap(~variable, scales = "free", labeller = labeller(variable = new_titles))
```
...
```{r}
dat_alpha = tibble(alpha = seq(0.01,0.2,length.out=50)) %>% mutate(dat = purrr::map(.x = alpha, .f = ~rphydro_nitrogen(tc, tc, ppfd, netrad, vpd, co2, pa, nitrogen, fapar, kphio, psi_soil, rdark, vwind, a_jmax, par_plant, list(alpha=.x, gamma=1, carbon_allocation = 1), options))) %>% unnest_wider(dat)
...
## TODO: Some values are values in the string bit, but not values in the StdValue section
data_try_jmax$OrigValueStr[data_try_jmax$TraitName == "Photosynthesis electron transport capacity (Jmax) per leaf nitrogen (N) content (Farquhar model)"]
data_try_jmax$StdValue[data_try_jmax$TraitName == "Photosynthesis electron transport capacity (Jmax) per leaf nitrogen (N) content (Farquhar model)"]
# Fix this!

out_data_try_pivot <- data_try %>%
...
# Create the plot
ggplot() +
  geom_point(data = out_data_try_pivot, aes(x = `Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf area at 25 C`,
                                            y = `Jmax per leaf area at ambient or leaf temperature`,
                                            color = `Latitude`)) +
  scale_color_viridis_c() +  # Color scale for the points
  labs(x = "Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf area at 25°C",
       y = "Jmax per leaf area at ambient or leaf temperature",
       color = "Latitude") +
  geom_line(data = dat, aes(y = jmax, x = vcmax, linetype = "Nitrogen Version, Allocation = 1"), color = "red") +
  geom_line(data = dat_2, aes(y = jmax, x = vcmax, linetype = "Nitrogen Version, Allocation = 2"), color = "blue") +
  geom_line(data = dat_1.5, aes(y = jmax, x = vcmax, linetype = "Nitrogen Version, Allocation = 1.5"), color = "green") +
  geom_line(data = dat_0.75, aes(y = jmax, x = vcmax, linetype = "Nitrogen Version, Allocation = 0.75"), color = "orange") +
...
```
## Jmax and leaf C/N ratio

```{r}

ggplot() +
    geom_point(data = scots_pine_pivot_std, aes(x = `Leaf carbon/nitrogen (C/N) ratio`,
                                                y = `Jmax per leaf area at ambient or leaf temperature`)) +
    labs(x = "Leaf carbon/nitrogen (C/N) ratio",
         y = "Jmax per leaf area at ambient or leaf temperature") +
    theme_minimal() +
    theme(legend.position = "right") + 
    coord_cartesian(xlim = c(min(scots_pine_pivot_std$`Leaf carbon/nitrogen (C/N) ratio`), 
                               max(scots_pine_pivot_std$`Leaf carbon/nitrogen (C/N) ratio`)),
                      ylim = c(min(scots_pine_pivot_std$`Photosynthetic electron transport capacity per leaf area at 25C (Jmax25area)`), 
...
ggplot() +
    geom_point(data = scots_pine_pivot_std, aes(x = `Leaf carbon/nitrogen (C/N) ratio`,
                                                y = `Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf dry mass at ambient temperature`)) +
    labs(x = "Leaf carbon/nitrogen (C/N) ratio",
         y = "Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf dry mass at ambient temperature") +
    theme_minimal() +
    theme(legend.position = "right") + 
    coord_cartesian(xlim = c(min(scots_pine_pivot_std$`Leaf carbon/nitrogen (C/N) ratio`), 
                               max(scots_pine_pivot_std$`Leaf carbon/nitrogen (C/N) ratio`)),
                      ylim = c(min(scots_pine_pivot_std$`Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf dry mass at ambient temperature`), 
                               max(scots_pine_pivot_std$`Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf dry mass at ambient temperature`))) + 
    geom_line(data = dat, aes(y = vcmax, x = n_leaf, linetype = "Nitrogen Version, Allocation = 1"), color = "red") +
    geom_line(data = dat_2, aes(y = vcmax, x = n_leaf, linetype = "Nitrogen Version, Allocation = 2"), color = "blue") +
    geom_line(data = dat_1.5, aes(y = vcmax, x = n_leaf, linetype = "Nitrogen Version, Allocation = 1.5"), color = "green") +
    geom_line(data = dat_0.75, aes(y = vcmax, x = n_leaf, linetype = "Nitrogen Version, Allocation = 0.75"), color = "orange") +
    scale_linetype_manual(name = "Nitrogen Allocation", 
                          values = c("Nitrogen Version, Allocation = 0.75" = "solid",
                                     "Nitrogen Version, Allocation = 1" = "solid",
                                     "Nitrogen Version, Allocation = 1.5" = "solid",
