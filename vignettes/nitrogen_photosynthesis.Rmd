...
par_cost = list(alpha=0.1, gamma=1, root_cost_per_zeta = 5);
...
ggplot() + 
  geom_line(data = df, aes(y = value, x = ppfd, color = "a = 1")) +
  geom_line(data = df_2, aes(y = value, x = ppfd, color = "a = 2")) +
  geom_line(data = df_1.5, aes(y = value, x = ppfd, color = "a = 1.5")) +
  geom_line(data = df_0.75, aes(y = value, x = ppfd, color = "a = 0.75")) +
  geom_line(data = df_original.m, aes(y = value, x = ppfd, color = "Numeric"), linetype="dotted") +
...
ggplot() + 
  geom_line(data = df_inst_0.75, aes(y = value, x = ppfd, color = "a = 0.75")) +
  geom_line(data = df_inst, aes(y = value, x = ppfd, color = "a = 1.0")) +
  geom_line(data = df_inst_1.5, aes(y = value, x = ppfd, color = "a = 1.5")) +
  geom_line(data = df_inst_2, aes(y = value, x = ppfd, color = "a = 2.0")) +
...
```{r}
dat_a_per_zeta = tibble(a_per_zeta = seq(0.01, 2, length.out=15)) %>% mutate(dat = purrr::map(.x = a_per_zeta, .f = ~rphydro_nitrogen(tc, tc, ppfd, netrad, vpd, co2, pa, nitrogen, fapar, kphio, psi_soil, rdark, vwind, a_jmax, par_plant, list(alpha=0.1, gamma=1, root_cost_per_zeta = .x), options))) %>% unnest_wider(dat)
...
```{r}
dat_a_jmax = tibble(a_jmax = seq(0.01,1,length.out=50)) %>% mutate(dat = purrr::map(.x = a_jmax, .f = ~rphydro_nitrogen(tc, tc, ppfd, netrad, vpd, co2, pa, nitrogen, fapar, kphio, psi_soil, rdark, vwind, .x, par_plant, par_cost, options))) %>% unnest_wider(dat)
...
```{r}
dat_alpha = tibble(alpha = seq(0.001,0.5,length.out=50)) %>% mutate(dat = purrr::map(.x = alpha, .f = ~rphydro_nitrogen(tc, tc, ppfd, netrad, vpd, co2, pa, nitrogen, fapar, kphio, psi_soil, rdark, vwind, a_jmax, par_plant, list(alpha=.x, gamma=1, root_cost_per_zeta = 5), options))) %>% unnest_wider(dat)
...
dat_nitrogen = tibble(nitrogen = seq(0.01,0.1,length.out=50)) %>% mutate(dat = purrr::map(.x = nitrogen, .f = ~rphydro_nitrogen(tc, tc, ppfd, netrad, vpd, co2, pa, .x, fapar, kphio, psi_soil, rdark, vwind, a_jmax, par_plant, par_cost, options))) %>% unnest_wider(dat)
