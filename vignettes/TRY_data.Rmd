...
psi_soil = -0.05
pa = calc_patm(elv)
nitrogen = 1 # TODO: this isn't doing anything at the moment
...
MESI_df <- a$mesi_main[a$mesi_main$ecosystem_type == "forest",] %>%
...
ggplot() +
  geom_point(data = results[nrow(results):1,], aes(y = vcmax25, x = jmax25, color = allocation), size = 2) +
  geom_point(data = colums_std, aes(x = `Photosynthetic electron transport capacity per leaf area at 25C (Jmax25area)`,
...
p <- ggplot() +
  geom_point(data = colums_std, aes(x = `Leaf nitrogen content per dry mass (Nmass)`/1000,
                                    ...
                                    shape = "TRY"), color = "grey", size = 1) +
  geom_smooth(data = colums_std, aes(x = `Leaf nitrogen content per dry mass (Nmass)`/1000,
                                    y = `Photosynthetic electron transport capacity per leaf area at 25C (Jmax25area)`,
                                    shape = "TRY"), 
              method = "lm", formula = y ~ 0 + x, se = FALSE, linetype = "dashed", color = "grey", size = 1) +
  ...
  geom_smooth(data = MESI_df, aes(x = leaf_n_mass,
                                  y = jmax,
                                  shape = "MESI"), 
              method = "lm", formula = y ~ 0 + x, se = FALSE, linetype = "dashed", color = "black", size = 1) +
  labs(x = "N per leaf (g g-1)",
  ...
  coord_cartesian(xlim = c(0, 0.075))  # This line sets the x-axis limits from 0 to 10
...
ggplot() +
  ...
  geom_point(data = colums_std, aes(x = `Leaf nitrogen content per dry mass (Nmass)`/1000,
                                    y = `Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf area at 25 C`,
                                    shape = "TRY"), color = "grey", size = 1, alpha = 0.5) +
