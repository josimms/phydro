...
ggplot() +
  ...
  guides(color = guide_legend(title = "Infrastructure"))
```
...
subtitle <- paste(
  ...
  create_colored_text("Allocation 0.75", gradient_0.75, "darkgreen"),
  create_colored_text("Allocation 1", gradient_1, "green"),
  create_colored_text("Allocation 1.5", gradient_1.5, "yellow"),
  create_colored_text("Allocation 2", gradient_2, "red"),
  sep = " | "
)
...
p <- ggplot() +
  ...
  labs(x = "N per leaf (g m-2)",
       ...
       title = "Gradients") +
  geom_line(data = dat_2, aes(y = jmax25, x = n_leaf, color = "Allocation = 2"), size = 1.2) +
  geom_line(data = dat_1.5, aes(y = jmax25, x = n_leaf, color = "Allocation = 1.5"), size = 1.2) +
  geom_line(data = dat, aes(y = jmax25, x = n_leaf, color = "Allocation = 1"), size = 1.2) +
  geom_line(data = dat_0.75, aes(y = jmax25, x = n_leaf, color = "Allocation = 0.75"), size = 1.2) +
  scale_color_manual(name = "Lines", 
                     values = c("Ambient Temperature" = "darkgrey",
                                "25°C" = "black",
                                "Allocation = 0.75" = "darkgreen",
                                "Allocation = 1" = "green",
                                "Allocation = 1.5" = "yellow",
                                "Allocation = 2" = "red")) +
  theme_minimal() +
  theme(strip.background = element_rect(color = "white", size = 1),
        legend.position = "right",
...
# Combine the plot and the subtitle
grid.arrange(p, subtitle_grob, ncol = 1, heights = c(10, 1))
```
## Vcmax and leaf C/N ratio

```{r}

# TODO: the units are not here?

results <- results %>%
  mutate(allocation = factor(allocation, levels = c(2.0, 1.5, 1.0, 0.75)))

# Now re-run the plot with this factor conversion
ggplot() +
  geom_point(data = scots_pine_colums_std, aes(x = `Leaf nitrogen content per area (Narea)`,
                                               y = `Photosynthetic carboxylation capacity (Vcmax, Vmax) per leaf area at leaf temperature`,
                                               linetype = "Area, Ambient Temperature"), col = "darkgrey") +
  ...
  geom_point(data = results, aes(y = vcmax25, x = n_leaf, color = allocation), size = 2) +
  theme_minimal() +
  theme(strip.background = element_rect(color = "white", size = 1),
        legend.title = element_blank(),
        legend.position = "right") +
